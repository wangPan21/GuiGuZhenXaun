<template>
  <div class="stats-container">
    ...
    <el-card class="box-cards">
      <el-form :inline="true">
        ...
        <el-form-item label="一级分类">
          <el-select placeholder="请选择" clearable v-model="category1Id" @change="reqCategoryone" :disabled="showbox == 3">
    ...
    <!-- 未选中数据展示页面 -->
    <div v-show="showbox == 1" style="display: flex; justify-content: center; align-items: center;">
      ...
      <el-card class="box-card">
        ...
        <div class="tables">
          ...
          <el-table style="width: 100%" border stripe :data="arrList">
            ...
            <el-table-column prop="attrName" label="属性名称" width="200" align="center"></el-table-column>
            <el-table-column label="属性值名称" align="center">
              <template #="{ row, $index }">
                <el-tag type="success" style="margin-left: 5px;" v-for="item in row.attrValueList">{{ item.valueName
                }}</el-tag>
              </template>
            </el-table-column>
            <el-table-column label="操作" width="140" align="center">
              <template #="{ row, $index }">
                ...
                <!-- 修改按钮的回调 -->
                <el-button type="warning" :icon="Edit" size="small" @click="editt(row)"> </el-button>
    ...
    <div v-show="showbox == 3" style="margin: 20px 0 0 0;">
      <el-card class="box-card">
          ...
          <div style="display: flex; padding-top: 10px;">
            <el-form :inline="true" :model="attrParams">
              <el-form-item label="属性名称">
                <el-input placeholder="请输入属性名称" v-model="attrParams.attrName" @blur="toTrim" />
              </el-form-item>
            </el-form>
            <el-button type="primary" :icon="Plus" @click="addAttrs" :disabled="trims">添加属性值</el-button>
          </div>
        </template>
        <div class="tables">

          <!-- 表单主体 -->
          <el-table style="width: 100%" border stripe :data="attrParams.attrValueList">
            <el-table-column label="序号" width="100" type="index" align="center"></el-table-column>
            <el-table-column label="属性值名称" align="center">
              <template #="{ row, $index }">
                <el-input placeholder="请输入属性值名称" v-model="row.valueName" v-if="row.flag" @blur="edits(row, $index)"
                  :ref="(vc: any) => inputArr[$index] = vc" />
                <div style="margin-left: 10px; text-align-last: left;" v-else @click="edits(row, $index)">{{ row.valueName
                }}
                </div>
              </template>
            </el-table-column>
            <el-table-column label="操作" width="140" align="center">
              <template #="{ row, $index }">
                <el-button type="danger" :icon="Delete" size="small" @click="attrParams.attrValueList.splice($index, 1)">
                </el-button>
              </template>
            </el-table-column>
          </el-table>
        ...
        <template #footer>
          <el-button type="primary" @click="submit" :disabled="trims">保存</el-button>
...
import { reqGetCategoryone, reqGetCategorytwo, reqGetCategorythree, reqAttrInfoList, reqAddOrEditAttr, reqDeleteAttr } from "@/api/product/index";
...
const reqCategorythree = async () => {
  ...
  showbox.value = 1
  //@ts-ignore
  let result4: AttrResponseData = await reqAttrInfoList(category1Id.value, category2Id.value, category3Id.value,);
  if (result4.code == 200) {
