        ...
        <el-form label-width="100px">
          ...
          <el-form-item label="spu品牌">
            <el-select placeholder="请选择spu品牌" v-model="SpuParams.tmId">
              <el-option v-for="(item, index) in spuAttr" :key="item.id" :label="item.tmName" :value="item.id" />
            </el-select>
          </el-form-item>
          ...
          <el-form-item label="spu照片">
            <el-upload v-model:file-list="spuImage" action="/api/admin/product/fileUpload" list-type="picture-card"
          ...
          <el-form-item label="spu销售属性">
            <el-select :placeholder="`还有${unSelectSaleAttr.length}个未选中`" v-model="unspuSales">
            ...
            <el-button type="primary" :icon="Plus" style="margin-left: 10px"
              :disabled="unSelectSaleAttr.length == 0 || unspuSales == undefined" @click="addSpuSales">
              添加属性
            </el-button>
          </el-form-item>
          <el-form-item>
            <el-table border :data="spuSales">
              ...
              <el-table-column prop="saleAttrName" label="属性名" width="120" align="center"></el-table-column>
              <el-table-column label="属性值" align="center">
                <template #="{ row, $index }">
                  <el-tag type="success" @close="row.spuSaleAttrValueList.splice($index, 1)"
                    v-for="item in row.spuSaleAttrValueList" :key="item.id" closable style="margin-right: 5px;">{{
                      item.saleAttrValueName }}</el-tag>
                  <el-input @blur="toLook(row)" v-model="row.saleAttrValue" v-if="row.flag == true" placeholder="请你输入属性值"
                    size="small" style="width: 20%;"></el-input>
                  <el-button v-else type="success" size="small" :icon="Plus" @click="addTag(row, $index)"></el-button>
                </template>
              </el-table-column>
              <el-table-column label="操作" width="120" align="center">
                <template #="{ row, $index }">
                  <el-button type="danger" size="small" :icon="Delete" @click="spuSales.splice($index, 1)"></el-button>
                </template>
              </el-table-column>
            </el-table>
          </el-form-item>
          <el-form-item>
            <el-button type="primary" @click="onSubmit" :disabled="spuSales.length > 0 ? false : true">保存</el-button>
