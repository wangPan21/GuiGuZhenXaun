<template>
  <div class="trademark-box">
    <el-card class="box-card">
      <template #header>
        <div class="card-header">
          ...
          <!-- 点击添加按钮 -->
          <el-button type="primary" :icon="Plus" @click="addTradmark">添加品牌</el-button>
        </div>
      </template>
      ...
      <div>
        <el-table border style="width: 100%" :data="recList">
          ...
          <el-table-column prop="tmName" label="品牌名称" width="250" align="center"></el-table-column>
          <el-table-column prop="prop" label="品牌LOGO" align="center">
            <template #="{ row, $index }">
              <img :src="row.logoUrl" alt="图片不存在" style="width: 100px; height: 100px;">
            </template>
          </el-table-column>
          <el-table-column prop="prop" label="品牌操作" width="180" align="center">
            <template #="{ row, $index }">
              ...
              <!-- 删除按钮 -->
              <el-popconfirm :title="`要删除品牌${row.tmName}吗？`" @confirm="removeTrademark(row.id)">
                <template #reference>
      ...
      <!-- 分页器  -->
      <template #footer>
        <el-pagination v-model:current-page="pageNo" v-model:page-size="limit" :page-sizes="[5, 9, 15, 20]"
    ...
    <!-- 对话框 -->
    <el-dialog v-model="showDialog" :title="trademarkInfo.id ? '修改品牌' : '添加品牌'" width="40%">

      <!-- 主体 -->
      <el-form status-icon style="width: 80%;" label-width="auto" :model="trademarkInfo" :rules="rules" ref="refFrom">
        <!-- 品牌名称 -->
...
import { reqTrademarkList, reqAddOrEditTrademark, reqRemoveTrademark } from "@/api/product/index";
